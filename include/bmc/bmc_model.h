// ============================================================================
// 文件功能描述：
// BMC模型（bmc_model）的头文件，定义BMC传感器相关的数据模型和结构。
// 主要功能包括：
// 1. UDP数据包模型：定义UdpInfo结构体，表示BMC通过UDP组播发送的完整数据包（风扇、传感器、板卡信息等）
// 2. 传感器数据模型：定义BMCSensorRow结构体，表示BMC传感器的时序数据
// 3. 数据包结构：定义UdpFanInfo、UdpSensorInfo等结构体，表示UDP数据包中的各个字段
