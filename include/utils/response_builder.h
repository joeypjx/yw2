// ============================================================================
// 文件功能描述：
// HTTP响应构建器（ResponseBuilder）的头文件，定义统一的HTTP响应格式化和发送接口。
// 主要功能包括：
// 1. 成功响应构建：提供sendSuccess方法，构建标准格式的成功响应
// 2. 错误响应构建：提供sendError方法，构建标准格式的错误响应
// 3. 旧版格式兼容：提供旧版API格式的响应构建方法，保持向后兼容
// 4. JSON格式化：支持紧凑格式和格式化输出（可配置缩进空格数）
// 5. 参数解析：提供解析HTTP请求参数的功能（hasParam、getParam、getIntParam等）
// 6. 工具方法：提供解析逗号分隔参数、去除空白字符等工具方法
// ============================================================================
