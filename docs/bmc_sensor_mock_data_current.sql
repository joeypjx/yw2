-- BMC传感器模拟数据插入脚本（当前时间）
-- 为指定主机和传感器生成当前时间的模拟数据

-- 生成函数：为指定主机和传感器生成当前时间的时序数据
DO $$
DECLARE
    host_ip TEXT;
    ...
    i INTEGER;
BEGIN
    -- 为每个主机生成数据（从过去1小时到现在，每10分钟一个数据点）
    FOR host_ip IN 
        SELECT unnest(ARRAY['192.168.1.100', '192.168.1.101', '192.168.1.102'])
    LOOP
        -- 温度传感器（3个）
        FOR i IN 1..3 LOOP
            FOR time_offset IN 0..6 LOOP
                sensor_time := NOW() - INTERVAL '1 hour' + (time_offset * INTERVAL '10 minutes');
...
FROM bmc_sensor
WHERE host_ip IN ('192.168.1.100', '192.168.1.101', '192.168.1.102')
GROUP BY host_ip
ORDER BY host_ip;
