...
CREATE TABLE IF NOT EXISTS bmc_fan (
    "timestamp" TIMESTAMPTZ NOT NULL, -- UdpInfo.timestamp
...
SELECT create_hypertable('bmc_fan', 'timestamp', if_not_exists => TRUE, migrate_data => TRUE);
...
CREATE INDEX IF NOT EXISTS idx_bmc_fan_box_time ON bmc_fan(boxid, "timestamp" DESC);
CREATE INDEX IF NOT EXISTS idx_bmc_fan_fanseq_time ON bmc_fan(fanseq, "timestamp" DESC);
...
SELECT create_hypertable('bmc_sensor', 'timestamp', if_not_exists => TRUE, migrate_data => TRUE);
...
CREATE INDEX IF NOT EXISTS idx_bmc_sensor_host_time ON bmc_sensor(host_ip, "timestamp" DESC);
CREATE INDEX IF NOT EXISTS idx_bmc_sensor_type_time ON bmc_sensor(sensortype, "timestamp" DESC);
CREATE INDEX IF NOT EXISTS idx_bmc_sensor_name_time ON bmc_sensor(sensorname, "timestamp" DESC);
