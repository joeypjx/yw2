// ============================================================================
// 文件功能描述：
// BMC路由（bmc_routes）的实现文件，提供BMC和板卡控制相关的HTTP API端点。
// 主要功能包括：
// 1. BMC信息查询：GET /bmc - 查询所有机箱的BMC信息
// 2. 机箱BMC查询：GET /bmc/{box_id} - 查询指定机箱的BMC信息
// 3. 板卡复位：POST /bmc/{box_id}/reset - 复位指定机箱的指定板卡
// 4. 板卡上电：POST /bmc/{box_id}/power-on - 开启指定机箱的指定板卡电源
// 5. 板卡下电：POST /bmc/{box_id}/power-off - 关闭指定机箱的指定板卡电源
// 6. IP地址计算：使用IPAddressUtils工具类根据机箱号和槽位号计算板卡IP地址
// 7. 参数验证：验证槽位ID范围（1-12），验证请求参数格式
// ============================================================================
