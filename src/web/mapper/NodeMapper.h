...
NodeView toNodeView(const node::NodeExt& ext, const monitor::Resource* res, const std::optional<std::uint8_t> prst = std::nullopt);
...
toHistoricalMetricsView(const monitor::ResourceWindow &win,
  ...
  if (bmc_sensor)
    v.metrics.sensor = *bmc_sensor;
  else
    v.metrics.sensor = nlohmann::json::object();
  return v;
}
