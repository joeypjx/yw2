...
std::shared_ptr<IWebModule> WebFactory::getWebModule(std::shared_ptr<hv::HttpServer> server,
    ...
    return std::make_shared<WebController>(std::move(server), std::move(service), std::move(node_module), std::move(monitor_module), std::move(bmc_module), std::move(alert_module));
}
