// ============================================================================
// 文件功能描述：
// BMC监听器（BMCListener）的实现文件，负责接收和处理BMC设备通过UDP组播发送的传感器数据。
// 主要功能包括：
// 1. UDP组播监听：创建UDP套接字，加入指定的组播组，接收BMC设备发送的数据包
// 2. 数据包解析：解析UDP数据包，验证包头包尾（0x5aa5），提取BMC传感器信息
// 3. 数据缓存：将接收到的BMC数据更新到内存缓存（BMCCache），提供快速查询
// 4. 数据持久化：将BMC数据保存到PostgreSQL数据库（TimescaleDB），用于历史查询
// 5. 回调处理：支持设置数据包处理回调函数，允许外部自定义处理逻辑
// 6. 查询接口：提供查询指定节点的BMC传感器时序数据和最新数据的功能
// 7. 板卡状态查询：提供查询指定机箱和板卡的在位状态（PRST）功能
// 8. 线程安全：使用独立线程运行接收循环，支持优雅启动和停止
// ============================================================================
