// ============================================================================
// 文件功能描述：
// IPMI客户端（IpmiClient）的实现文件，封装FreeIPMI库，提供IPMI协议通信功能。
// 主要功能包括：
// 1. IPMI连接管理：使用FreeIPMI库建立RMCP+ 2.0协议的IPMI连接，支持加密认证
// 2. 会话管理：管理IPMI会话的生命周期，包括创建、认证、超时和关闭
// 3. 原始命令发送：提供sendRaw方法，支持发送任意IPMI原始命令（网络功能号+命令码+数据）
// 4. IPMB桥接：通过IPMB（Intelligent Platform Management Bus）桥接方式发送命令
// 5. 响应解析：解析IPMI响应数据，检查完成码（Completion Code），提取有效负载
// 6. 错误处理：捕获和转换FreeIPMI库的错误信息，提供友好的错误消息
// 7. 资源管理：在析构时自动关闭IPMI连接和清理FreeIPMI上下文资源
// 8. 配置选项：支持配置主机名、用户名、密码、权限级别、加密套件、超时等参数
// ============================================================================
