// ============================================================================
// 文件功能描述：
// 告警规则（AlertRule）域模型的实现文件，定义告警规则的数据结构和行为。
// 主要功能包括：
// 1. 规则定义：定义告警规则的属性（名称、表达式、持续时间、严重程度、摘要、描述等）
// 2. JSON序列化：提供toJson和fromJson方法，支持规则的JSON格式转换
// 3. ID生成：自动生成唯一的告警规则ID（格式：rule_时间戳_随机数）
// 4. 时间戳管理：自动管理创建时间和更新时间
// 5. 规则验证：验证告警规则的有效性（ID、名称、表达式等必填字段）
// 6. 表达式管理：管理告警表达式（包含条件、指标、表名、标签等）
// ============================================================================
