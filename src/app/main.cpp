// ============================================================================
// 文件功能描述：
// 这是应用程序的主入口文件，负责整个系统的启动、初始化和生命周期管理。
// 主要功能包括：
// 1. 日志系统初始化：配置spdlog日志库，支持控制台和文件输出，支持日志轮转
// 2. 应用上下文创建：创建AppContext实例，管理HTTP服务器和所有业务模块
// 3. 模块创建与注册：创建节点模块、监控模块、BMC模块、控制器模块和告警模块
// 4. 信号处理：注册SIGINT和SIGTERM信号处理器，实现优雅关闭
// 5. 主循环：运行主事件循环，保持应用程序运行直到收到停止信号
// 6. 资源清理：在程序退出时自动清理所有模块和HTTP服务器资源
// ============================================================================
