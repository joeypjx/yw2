...
MetricsSeries ResourceRepository::queryMetricsSeries(const std::string& host_ip,
    ...
    // 将时间戳转换为 PostgreSQL 时间戳
    std::string start_ts = "to_timestamp(" + std::to_string(start_time) + ")";
    std::string end_ts = "to_timestamp(" + std::to_string(end_time) + ")";
    ...
    if (query_all || need.count("cpu")) {
        pqxx::result r = tx.exec_params(
            ...
            "FROM ( "
            "    SELECT "
            "        time_bucket_gapfill('10 seconds', time, to_timestamp($2), to_timestamp($3)) AS bucket, "
            "        AVG(usage_percent) as usage_percent, "
            "        AVG(load_avg_1m) as load_avg_1m, "
            "        AVG(load_avg_5m) as load_avg_5m, "
            "        AVG(load_avg_15m) as load_avg_15m, "
...
WITH bucket_series AS (
  SELECT generate_series(
    time_bucket('10 seconds', to_timestamp($2)),
    time_bucket('10 seconds', to_timestamp($3)),
    '10 seconds'::interval
  ) AS bucket
),
interface_dims AS (
...
ON
    ...
    AND metrics.host_ip = $1::inet
    AND time_bucket('10 seconds', metrics.time) = buckets.bucket
    AND metrics.time >= to_timestamp($2) AND metrics.time <= to_timestamp($3)
GROUP BY
    dims.interface, buckets.bucket
ORDER BY
    dims.interface, ts ASC
