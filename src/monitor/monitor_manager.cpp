// ============================================================================
// 文件功能描述：
// 监控管理器（MonitorManager）的实现文件，负责管理计算节点的资源监控数据。
// 主要功能包括：
// 1. 资源数据接收：接收节点通过POST /resource发送的资源监控数据（CPU、内存、磁盘、网络、GPU等）
// 2. 资源缓存管理：使用MonitorCache维护最新资源数据的内存缓存，提供快速查询
// 3. 时序数据查询：查询指定节点在指定时间范围内的历史监控指标数据（支持多种指标类型）
// 4. 数据导出功能：导出节点历史数据，将不同指标类型的数据按时间戳对齐合并
// 5. 数据持久化：将资源监控数据保存到PostgreSQL数据库（TimescaleDB），用于历史查询和分析
// 6. 资源扫描器：启动MulticastScanner，通过组播方式主动发现和收集节点资源数据
// 7. 数据验证：验证资源请求中的数据格式和有效性（IP地址格式等）
// 8. HTTP路由注册：注册/resource端点，处理节点资源数据上报请求
// ============================================================================
