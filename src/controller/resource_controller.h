// ============================================================================
// 文件功能描述：
// 资源控制器（ResourceController）的头文件，定义板卡控制服务的接口和实现。
// 主要功能包括：
// 1. 板卡控制：实现IControllerModule接口，提供板卡复位、上电、下电等控制操作
// 2. TCP通信：通过TCP协议连接到目标机箱，发送二进制控制命令并接收响应
// 3. 超时控制：使用非阻塞socket和select实现连接超时，使用SO_SNDTIMEO和SO_RCVTIMEO实现收发超时
// 4. 命令构建：将操作命令、目标IP、槽位列表等信息封装为二进制操作模型结构体
// 5. 响应解析：解析机箱返回的二进制响应数据，提取每个槽位的操作结果和状态
// 6. 错误处理：处理网络错误、超时错误、响应格式错误等各种异常情况
// ============================================================================
