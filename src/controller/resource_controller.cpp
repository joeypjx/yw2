// ============================================================================
// 文件功能描述：
// 资源控制器（ResourceController）的实现文件，负责通过TCP协议控制机箱板卡的电源和复位操作。
// 主要功能包括：
// 1. 板卡复位：通过TCP发送RESET命令，重置指定机箱的指定板卡
// 2. 板卡上电：通过TCP发送POWON命令，开启指定机箱的指定板卡电源
// 3. 板卡下电：通过TCP发送POWOFF命令，关闭指定机箱的指定板卡电源
// 4. TCP通信：建立TCP连接到目标机箱的33000端口，发送二进制控制命令并接收响应
// 5. 超时控制：使用非阻塞socket和select实现连接超时，使用SO_SNDTIMEO和SO_RCVTIMEO实现收发超时
// 6. 命令构建：将操作命令、目标IP、槽位列表等信息封装为二进制操作模型结构体
// 7. 响应解析：解析机箱返回的二进制响应数据，提取每个槽位的操作结果和状态
// 8. 错误处理：处理网络错误、超时错误、响应格式错误等各种异常情况
// ============================================================================
