// ============================================================================
// 文件功能描述：
// 持续时间工具类（DurationUtils）的实现文件，提供持续时间字符串的解析和转换功能。
// 主要功能包括：
// 1. 持续时间解析：解析简写格式的持续时间字符串（如"5s"、"10m"、"2h"）
// 2. 单位转换：支持秒（s）、分钟（m）、小时（h）三种时间单位
// 3. 毫秒转换：将持续时间字符串转换为毫秒数，用于定时器和延迟操作
// 4. PostgreSQL格式转换：将持续时间转换为PostgreSQL INTERVAL格式，用于数据库查询
// 5. 秒数转换：将持续时间字符串转换为秒数，用于时间计算
// 6. 默认值支持：解析失败时返回指定的默认值，提高容错性
// ============================================================================
